...
async function refresh() {
  try {
    const [data, live, goalsMap, bracket] = await Promise.all([
      loadSheet(), fetchLive(), fetchGoalsMap(), fetchBracket(),
    ]);
    ...
    setStatus(liveCount
      ...
      : `Uppdaterad ${time()}`);
    renderHero(heroEl, state.liveEnriched, data, state.bracket);
    render();
  } catch (err) {
